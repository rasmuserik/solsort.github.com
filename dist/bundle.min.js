...
bundler.module('scripts/fullbrows.js','var util=require("util"),webutil=require("webutil"),$=require("zquery"),window=require("window"),Modernizr=require("modernizr"),relayoutFn,browsOpt;function relayout(){$("#content").css("position","absolute").css("left",0).css("top",1).css("overflow","hidden").css("font-family","sans-serif").css("width",$(window).width()).css("height",browsOpt.scrollable?"auto":webutil.windowHeight()),typeof relayoutFn=="function"&&relayoutFn(window.document.getElementById("content")),window.scrollTo(0,1)}var relayoutDelayed=util.niceSingle(relayout),init=exports.init=function(opt){browsOpt=opt||{},relayoutFn=browsOpt.callback||browsOpt.update,window.document.getElementById("content")||$("body").append(\x27<div id="content"></div>\x27),Modernizr.touch?$("#addressbarfillerdisable").length===0&&$("body").append(\x27<div id="addressbarfillerdisable" style="height:\x27+(Math.max(webutil.windowHeight(),$(window).width())+62)+\x27px;"></div>\x27):$("body").css("overflow","hidden"),$(window).bind("resize",relayoutDelayed),$(window).bind("orientationchange",relayoutDelayed),relayout()};exports.layoutFunction=function(fn){init({callback:fn})}');

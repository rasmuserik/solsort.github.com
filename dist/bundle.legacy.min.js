...
bundler.module('scripts/julia4d.js','exports.app={underbar:!0,type:"canvas",start:function(){c=this.elem,a=c.getContext("2d"),c.width=796,c.height=512;var d,p,e,vs,k,pts,w0,wi0,w1,wi1,pts0,kcount,w,wi,j,Cx,Cy,v1,v2,x,y,i;d=a.getImageData(0,0,c.width,c.height),e=d.data,vs=1;function blit(){a.putImageData(d,0,0)}function setp(x,y,e){d.data[((y|0)*c.width+(x|0))*4+0]=255&e,d.data[(y*c.width+x)*4+3]=255}function incp(x,y,e){0<x&&x<c.width&&(d.data[((y|0)*c.width+(x|0))*4+0]+=50)}function fade(){for(var i=e.length;i>0;i-=4)e[i-1]=255,e[i-2]=255&e[i-2]*.999,e[i-3]=255&e[i-3]*.999,e[i-4]=255&e[i-4]*.999}function clear(){for(var i=0;i<e.length;i+=4)e[i]=0,e[i+3]=255}k=0,pts=[],w0=Math.random()*(Math.random()-Math.random()),wi0=Math.random()*(Math.random()-Math.random()),w1=Math.random()*(Math.random()-Math.random()),wi1=Math.random()*(Math.random()-Math.random()),w0=wi0=w1=wi1=0,pts0=[],kcount=3e4,newpts();function newpts(){w0+=Math.random()*-0.5,wi0+=Math.random()*-0.5,w1+=Math.random()*-0.5,wi1+=Math.random()*-0.5,k=0,setTimeout(drawpts,0)}function sign(x){return x>0?1:-1}function drawpts(){for(;;){var z,zi,t,ti,r,s,Newz,Newzi,random=Math.random,sqrt=Math.sqrt;w=k/kcount*w0+(1-k/kcount)*w1,wi=k/kcount*wi0+(1-k/kcount)*wi1,z=random()*2-1,zi=random()*2-1;for(j=0;j<20;++j)Cx=w,Cy=wi,z-=Cx,zi-=Cy,z>0?(Newz=sqrt((z+sqrt(z*z+zi*zi))/2),Newzi=zi/(2*Newz)):z<0?(Newzi=sign(zi)*sqrt((-z+sqrt(z*z+zi*zi))/2),Newz=zi/(2*Newzi)):(Newz=sqrt(Math.abs(zi)/2),Newz>0?Newzi=zi/(2*Newz):Newzi=0),random()<.5?(z=Newz,zi=Newzi):(z=-Newz,zi=-Newzi),pts[k]={x:z/8,y:zi/8,z:k/kcount/2-.25};++k;if(k%300===0){k<kcount?setTimeout(drawpts,0):setTimeout(newpts,0);return}}}d3();function transform(pt){v1=.005*vs,v2=.01*vs;var xa=pt.x*Math.cos(v1)-pt.y*Math.sin(v1),ya=pt.x*Math.sin(v1)+pt.y*Math.cos(v1),za=xa*Math.sin(v2)+pt.z*Math.cos(v2);return xa=xa*Math.cos(v2)-pt.z*Math.sin(v2),{x:xa,y:ya,z:za}}function d3(){clear();for(var i=0;i<pts.length;++i)p=pts[i],p=transform(p),incp((p.x+.5)*c.width,(p.y+.5)*c.height);++vs,blit(),setTimeout(d3,0)}x=y=100;function f(){for(var i=0;i<1e4;++i)x=Math.abs(x%c.width)+Math.random()-.5,y=Math.abs(y%c.height)+Math.random()-.5,d.data[((y|0)*c.width+(x|0))*4+0]+=1;fade(),blit(),setTimeout(f,100)}for(i=0;i<d.data.length;++i);a.putImageData(d,0,0)},stop:function(){var i,t;t=setTimeout(function(){},1e4);for(i=0;i<=t;++i)clearTimeout(i);t=setInterval(function(){},1e4);for(i=0;i<=t;++i)clearInterval(i)}}');

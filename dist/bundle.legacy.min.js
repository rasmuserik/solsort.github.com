...
bundler.module('scripts/main.js','var $=require("zquery"),fullbrows=require("fullbrows"),notes={type:"scrollable",start:function(){var showdown;showdown=require("showdown"),showdown=new showdown.converter;var app=this;$.get("notes/"+this.param+".md",function(text){var html=showdown.makeHtml(text);app.$.html(html)})}};$("head").append(\x27<style>@font-face { font-family: Ubuntu; src: url("/images/Ubuntu-R.ttf"); }@font-face { font-family: sans-serif; src: url("/images/Ubuntu-R.ttf"); }@font-face { font-family: "Ubuntu Mono"; src: url("/images/UbuntuMono-R.ttf"); }@font-face { font-family: "Ubuntu Condensed"; src: url("/images/Ubuntu-C.ttf"); }tt { font-family: "Ubuntu Mono", monospace; }code { font-family: "Ubuntu Mono", monospace; }pre { font-family: "Ubuntu Mono", monospace; }body { font-family: Ubuntu, sans-serif; }</style>\x27);var menuXml=["ul",["li","solsort.dk",["ul",["li",["a",{href:"/CurriculumVitae.html"},"CV"]],["li","Apps",["ul",["li",["a",{href:"/dkcities"},"Danskebyer"]],["li",["a",{href:"/notescore"},"notescore"]],["li",["a",{href:"/planetcute"},"PlanetCute"]]]],["li","Notes",["ul",["li",["a",{href:"/notes/tekststruktur-for-rapporter.html"},"Tekststruktur for rapporter"]],["li",["a",{href:"/notes/tommelfingerregler-for-skrivning.html"},"Tommelfingerregler for skrivning"]],["li",["a",{href:"/notes/fototips.html"},"Fototips"]],["li","Tech",["ul",["li",["a",{href:"/notes/tech/c9ender.html"},"Cloud9 and Ender"]],["li",["a",{href:"/notes/tech/jekyll.html"},"Jekyll / github-pages"]],["li",["a",{href:"/notes/tech/vim.html"},"vim"]],["li",["a",{href:"/notes/tech/coffee.html"},"CoffeeScript"]]]],["li",["a",{href:"/notes/ted.html"},"TED talks"]]]],["li","JS",["ul",["li",["a",{href:"/devintro/book.html"},"devintro"]],["li","1k",["ul",["li",["a",{href:"/f1k/blah.html"},"4d juliabrot"]],["li",["a",{href:"/f1k/rain.html"},"rain"]]]],["li","tweetsize",["ul",["li",["a",{href:"/f1k/brown.html"},"brownian motion"]],["li",["a",{href:"/f1k/sierpinsky.html"},"sierpinsky"]]]],["li","\u201ciOS style\u201d:"],["li","\u201chierachical layout\u201d:"],["li","\u201cslidein transitions\u201d:"]]],["li","Presentations",["ul",["li",["a",{href:"/presentations/oauth2.html"},"OAuth2"]]]]]]],currentName="";function dispatch(){var param=window.location.hash.slice(1);for(var name in apps)if(param.slice(0,name.length)===name){param=param.slice(name.length);break}console.log("name:",name,"param:",param),console.log(apps[name]),apps[name].param=param,fullbrows.start(apps[name])}exports.main=function(){$(function(){window.onhashchange=dispatch,dispatch()})},exports.switchTo=function(name,param){name||(name=currentName),window.location.hash=name+param};function unicodeTest(){var t=[],i,n=65538;$("body").text("");for(i=0;i<n;++i)t.push(i);t=t.map(function(a){return String.fromCharCode(a)}),t=t.join("");for(i=0;i<n;++i)t.charCodeAt(i)!==i&&$("body").append("error t["+i+"] -> "+t.charCodeAt(i)+"<br>");$("body").append("done")}var apps={combigame:require("combigame").app,bidiv:require("bidiv").app,plasma:require("demoPlasma").app,"source/":require("showSource").app,timelog:require("timelog").app,"js1k/":require("js1k").app,"notes/":notes,menu:require("./menu").createApp(menuXml),"default":{start:function(){}}}');
...
bundler.module('scripts/js1k.js','exports.app={type:"canvas",start:function(){c=this.elem,a=c.getContext("2d"),js1k[this.param]()}};var js1k={rain:function(){var d,e,g,h,x,i,j,t;c.width=256,c.height=256,d=a.getImageData(0,0,c.width,c.height),e=d.data;function blit(){a.putImageData(d,0,0)}g=[],h=[];for(x=0;x<1e4;++x);function f(){for(i=0;i<65536;++i)h[i]=((g[i-1]+g[i+1]+g[i-256]+g[i+256])/2-h[i])*.99||0,e[i*4+3]=255,e[i*4+2]=255&128+g[i]-g[i-257];j=g,g=h,h=j,t=Math.random()*65536|0,g[t]=127,g[t+1]=127,g[t+2]=127,g[256+t+1]=127,g[512+t+1]=127,blit(),setTimeout(f,0)}f()},sierp:function(){c.width=c.height=1e3;var b,i,t=1e3;for(b=[i=1];i<t*t;)(b[i+t]=b[i]^b[i++ -1])&&a.fillRect(i%t,i/t,1,1)},brown:function(){c.width=c.height=128,d=a.getImageData(p=0,0,128,128),setInterval("for(i=k=66055;--i;d.data[p%k]++)p+=Math.random()*k*k&k;a.putImageData(d,0,0)",50)},julia4d:function(){c.width=796,c.height=512;var d,p,e,vs,k,pts,w0,wi0,w1,wi1,pts0,kcount,w,wi,j,Cx,Cy,v1,v2,x,y,i;d=a.getImageData(0,0,c.width,c.height),e=d.data,vs=1;function blit(){a.putImageData(d,0,0)}function setp(x,y,e){d.data[((y|0)*c.width+(x|0))*4+0]=255&e,d.data[(y*c.width+x)*4+3]=255}function incp(x,y,e){0<x&&x<c.width&&(d.data[((y|0)*c.width+(x|0))*4+0]+=50)}function fade(){for(var i=e.length;i>0;i-=4)e[i-1]=255,e[i-2]=255&e[i-2]*.999,e[i-3]=255&e[i-3]*.999,e[i-4]=255&e[i-4]*.999}function clear(){for(var i=0;i<e.length;i+=4)e[i]=0,e[i+3]=255}k=0,pts=[],w0=Math.random()*(Math.random()-Math.random()),wi0=Math.random()*(Math.random()-Math.random()),w1=Math.random()*(Math.random()-Math.random()),wi1=Math.random()*(Math.random()-Math.random()),w0=wi0=w1=wi1=0,pts0=[],kcount=3e4,newpts();function newpts(){w0+=Math.random()*-0.5,wi0+=Math.random()*-0.5,w1+=Math.random()*-0.5,wi1+=Math.random()*-0.5,k=0,setTimeout(drawpts,0)}function sign(x){return x>0?1:-1}function drawpts(){for(;;){var z,zi,t,ti,r,s,Newz,Newzi,random=Math.random,sqrt=Math.sqrt;w=k/kcount*w0+(1-k/kcount)*w1,wi=k/kcount*wi0+(1-k/kcount)*wi1,z=random()*2-1,zi=random()*2-1;for(j=0;j<20;++j)Cx=w,Cy=wi,z-=Cx,zi-=Cy,z>0?(Newz=sqrt((z+sqrt(z*z+zi*zi))/2),Newzi=zi/(2*Newz)):z<0?(Newzi=sign(zi)*sqrt((-z+sqrt(z*z+zi*zi))/2),Newz=zi/(2*Newzi)):(Newz=sqrt(Math.abs(zi)/2),Newz>0?Newzi=zi/(2*Newz):Newzi=0),random()<.5?(z=Newz,zi=Newzi):(z=-Newz,zi=-Newzi),pts[k]={x:z/8,y:zi/8,z:k/kcount/2-.25};++k;if(k%300===0){k<kcount?setTimeout(drawpts,0):setTimeout(newpts,0);return}}}d3();function transform(pt){v1=.005*vs,v2=.01*vs;var xa=pt.x*Math.cos(v1)-pt.y*Math.sin(v1),ya=pt.x*Math.sin(v1)+pt.y*Math.cos(v1),za=xa*Math.sin(v2)+pt.z*Math.cos(v2);return xa=xa*Math.cos(v2)-pt.z*Math.sin(v2),{x:xa,y:ya,z:za}}function d3(){clear();for(var i=0;i<pts.length;++i)p=pts[i],p=transform(p),incp((p.x+.5)*c.width,(p.y+.5)*c.height);++vs,blit(),setTimeout(d3,0)}x=y=100;function f(){for(var i=0;i<1e4;++i)x=Math.abs(x%c.width)+Math.random()-.5,y=Math.abs(y%c.height)+Math.random()-.5,d.data[((y|0)*c.width+(x|0))*4+0]+=1;fade(),blit(),setTimeout(f,100)}for(i=0;i<d.data.length;++i);a.putImageData(d,0,0)}}');
bundler.module('scripts/demoPlasma.js','exports.app={type:"canvas",update:function(){var canvas=this.elem;canvas.width=canvas.height=512;var ctx=canvas.getContext("2d"),img=ctx.createImageData(512,512),x,y,col=0;function get(x,y){return img.data[((x&511)+(y&511)*512)*4+col]}function put(x,y,c){img.data[((x&511)+(y&511)*512)*4+col]=c}var i=0;for(i=0;i<img.data.length>>2;++i)img.data[i*4+3]=255;var step=512;function colrand(){return(Math.random()-.5)*step*3}function plasma(){while(step>0){var prevStepMask=(step<<1)-1;for(x=0;x<512;x+=step)for(y=0;y<512;y+=step)y&prevStepMask&&x&prevStepMask&&(col=0,put(x,y,(get(x-step,y-step)+get(x+step,y+step)+get(x+step,y-step)+get(x-step,y+step))/4+colrand()),col=1,put(x,y,(get(x-step,y-step)+get(x+step,y+step)+get(x+step,y-step)+get(x-step,y+step))/4+colrand()),col=2,put(x,y,(get(x-step,y-step)+get(x+step,y+step)+get(x+step,y-step)+get(x-step,y+step))/4+colrand()));for(x=0;x<512;x+=step)for(y=0;y<512;y+=step)if(!(y&prevStepMask)&&x&prevStepMask||y&prevStepMask&&!(x&prevStepMask))col=0,put(x,y,(get(x-step,y)+get(x+step,y)+get(x,y-step)+get(x,y+step))/4+colrand()),col=1,put(x,y,(get(x-step,y)+get(x+step,y)+get(x,y-step)+get(x,y+step))/4+colrand()),col=2,put(x,y,(get(x-step,y)+get(x+step,y)+get(x,y-step)+get(x,y+step))/4+colrand());step>>=1}}plasma(),col=1,plasma(),col=2,plasma(),ctx.putImageData(img,0,0)}}');

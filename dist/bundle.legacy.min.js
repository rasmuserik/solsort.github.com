...
bundler.module('scripts/rain.js','var running;exports.app={underbar:!0,type:"canvas",start:function(){running=!0;var c=this.elem,a=c.getContext("2d"),size=256;c.width=c.height=size;var image=a.getImageData(0,0,c.width,c.height),imageBuffer=image.data,g=[],h=[];f();function f(){running&&setTimeout(f,0);var pos=Math.random()*size*size|0;g[pos]=127,g[pos+1]=127,g[pos+2]=127,g[size+pos+1]=127,g[2*size+pos+1]=127;var t=g;g=h,h=t;for(var i=0;i<size*size;++i)h[i]=((g[i-1]+g[i+1]+g[i-size]+g[i+size])/2-h[i])*.99||0,imageBuffer[i*4+3]=255,imageBuffer[i*4+2]=255&128+h[i]-h[i-size-1];a.putImageData(image,0,0)}},stop:function(){running=!1}}');

...
bundler.module('scripts/main.js','var $=require("zquery"),fullbrows=require("fullbrows"),notes={type:"scrollable",start:function(){var showdown;showdown=require("showdown"),showdown=new showdown.converter;var app=this;$.get("notes/"+this.param+".md",function(text){var html=showdown.makeHtml(text);app.$.html(html)})}};$("head").append(\x27<style>@font-face { font-family: Ubuntu; src: url("img/Ubuntu-R.ttf"); }@font-face { font-family: sans-serif; src: url("img/Ubuntu-R.ttf"); }@font-face { font-family: "Ubuntu Mono"; src: url("img/UbuntuMono-R.ttf"); }@font-face { font-family: "Ubuntu Condensed"; src: url("img/Ubuntu-C.ttf"); }tt { font-family: "Ubuntu Mono", monospace; }code { font-family: "Ubuntu Mono", monospace; }pre { font-family: "Ubuntu Mono", monospace; }body { font-family: Ubuntu, sans-serif; }</style>\x27);var menuXml=["ul",["li","solsort",["ul",["li",["a",{href:"#rasmuserik"},"Rasmus Erik"]],["li","Apps",["ul",["li",["a",{href:"#dkcities"},"Danske Byer"]],["li",["a",{href:"#notescore"},"Note\xadScore"]],["li",["a",{href:"#planetcute"},"Planet\xadCute"]],["li",["a",{href:"#combigame"},"Combi\xadgame"]]]],["li","Notes",["ul",["li",["a",{href:"#notes/tekststruktur-for-rapporter"},"Tekst\xadstruktur for rapporter"]],["li",["a",{href:"#notes/tommelfingerregler-for-skrivning"},"Tommel\xadfinger\xadregler for skrivning"]],["li",["a",{href:"#notes/fototips"},"Foto\xadtips"]],["li",["a",{href:"#notes/speak-eval"},"Evalu\xadering af taler"]]]],["li","Images",["ul"]],["li","Code Demos",["ul",["li",["a",{href:"#plasma"},"Diamond-square plasma generation"]],["li",["a",{href:"#js1k/sierp"},"Tweetsize Sierpinsky tri\xadangle"]],["li",["a",{href:"#js1k/brown"},"Tweetsize brownian noise animation"]],["li",["a",{href:"#js1k/rain"},"Rain\xaddrops on water simulation"]],["li",["a",{href:"#js1k/julia4d"},"Projection of 4d-julia\xadbrot fractal"]]]]]]],currentName="";function dispatch(){var param=window.location.hash.slice(1);for(var name in apps)if(param.slice(0,name.length)===name){param=param.slice(name.length);break}console.log("name:",name,"param:",param),console.log(apps[name]),apps[name].param=param,fullbrows.start(apps[name])}exports.main=function(){$(function(){window.onhashchange=dispatch,dispatch()})},exports.switchTo=function(name,param){name||(name=currentName),window.location.hash=name+param};function unicodeTest(){var t=[],i,n=65538;$("body").text("");for(i=0;i<n;++i)t.push(i);t=t.map(function(a){return String.fromCharCode(a)}),t=t.join("");for(i=0;i<n;++i)t.charCodeAt(i)!==i&&$("body").append("error t["+i+"] -> "+t.charCodeAt(i)+"<br>");$("body").append("done")}var apps={combigame:require("combigame").app,bidiv:require("bidiv").app,notescore:require("notescore").app,dkcities:require("dkcities").app,plasma:require("demoPlasma").app,"source/":require("showSource").app,timelog:require("timelog").app,"js1k/":require("js1k").app,"notes/":notes,menu:require("./menu").createApp(menuXml),"default":{start:function(){}}}');
bundler.module('scripts/showSource.js','var $=require("zquery"),_=require("underscore"),console=require("console"),doccoLineRegEx=/^\x5cs*\x5c/\x5c/( |$)/;function isDoccoComment(line){return line.match(doccoLineRegEx)}function docco(text){var i,showdown=require("showdown");showdown=new showdown.converter,text=text.split("\x5cn");var result=[],current=[],prevIsDocco=!0,lineno=0;text.forEach(function(line){var currentIsDocco=!!isDoccoComment(line);currentIsDocco?line=line.replace(doccoLineRegEx,""):line={lineno:lineno,line:line},prevIsDocco===currentIsDocco?current.push(line):(result.push(current),current=[line]),prevIsDocco=currentIsDocco,++lineno}),result.push(current),result.length&1&&result.push([]);var docs=[],code=[];for(i=0;i<result.length;i+=2)docs.push(result[i]),code.push(result[i+1]);docs=docs.map(function(doc){return showdown.makeHtml(doc.join("\x5cn"))}),code=code.map(function(code){return"<pre>"+_.escape(_.pluck(code,"line").join("\x5cn"))+"</pre>"}),result=["<table>"];for(i=0;i<docs.length;++i)result.push(\x27<tr><td style="vertical-align: top">\x27),result.push(docs[i]),result.push(\x27</td><td style="vertical-align: top">\x27),result.push(code[i]),result.push("</td></tr>");return result.push("</table>"),result.join("")}exports.app={type:"scrollable",start:function(){var app=this;$.ajax({url:"/scripts/"+this.param+".js",dataType:"text",success:function(text){app.$.html(docco(text))}})}}');
bundler.module('scripts/menu.js','var util=require("util"),fullbrows=require("fullbrows"),Modernizr=require("modernizr"),$=require("zquery"),window=require("window"),boxWidth=1.5,position,positionArray,margin=2,padding=2,titlesize=20,border=Modernizr.boxshadow?0:1;function elemToObj(elem){var result={};result.title=$(elem.firstChild).text().trim(),result.children=[];if(elem.children[0]){var elems=elem.children[0].children;for(var i=0;i<elems.length;++i)result.children.push(elemToObj(elems[i]))}return result.url=elem.children[0]&&elem.children[0].href,result.elem=elem,result}function totalSize(arr){return arr.reduce(function(a,b){return a+b.size},0)}function initMenu(menu){var style=menu.elem.style;style.fontSize="16px",style.display="block",style.position="absolute",style.padding=padding+"px",style.top="100px",style.left="100px",style.margin=margin+"px",style.textAlign="center",style.borderRadius=margin*2+"px",style.border=border+"px solid #000000",style.overflow="hidden",style.backgroundColor=util.colorHash(menu.title),style.boxShadow="3px 3px 9px rgba(0, 0, 0, .8)",style.webkitBoxShadow="3px 3px 9px rgba(0, 0, 0, .8)";if(menu.url){var url=menu.url;$(menu.elem).bind(Modernizr.touch?"touchstart":"mousedown",function(){window.location=url})}menu.children.forEach(initMenu),menu.size=totalSize(menu.children)/1.5+1}exports.createApp=function(jsonml){var menu;return{start:function(){$("#content").html(require("jsxml").toDOM(jsonml)),menu=elemToObj($("#content > ul > li")[0]),initMenu(menu),this.update(this.elem)},update:function(){var elem=this.elem;position(menu,-margin,-margin,$(elem).width()+margin,$(elem).height()+margin)}}},position=function(menu,x,y,w,h){w-=2*(margin+border+padding),h-=2*(margin+border+padding);var $elem=$(menu.elem);$elem.css("left",x).css("top",y).css("width",w);var fontsize=16;$elem.css("fontSize",fontsize);if(menu.children.length===0){$elem.css("height","auto");while(fontsize&&$elem.height()>h)--fontsize,$elem.css("fontSize",fontsize)}$elem.css("height",h),$elem.css("boxShadow","3px 3px 9px rgba(0, 0, 0, .8)"),positionArray(menu.children,padding*2,titlesize+padding,w-padding*2,h-titlesize-padding*2)},positionArray=function(arr,x,y,w,h){if(arr.length===0)return;if(arr.length===1)return position(arr[0],x,y,w,h);var weight=totalSize(arr),splitsum=weight,sum=0,pos=0,ratio;for(var i=0;i<arr.length;++i)sum+=arr[i].size,Math.abs(sum-weight/2)<splitsum&&(pos=i+1,splitsum=Math.abs(sum-weight/2),ratio=sum/weight);var arr1=arr.slice(0,pos),arr2=arr.slice(pos);w>boxWidth*h?(positionArray(arr1,x,y,w*ratio|0,h),positionArray(arr2,x+w*ratio|0,y,w-(w*ratio|0),h)):(positionArray(arr1,x,y,w,h*ratio|0),positionArray(arr2,x,y+h*ratio|0,w,h-(h*ratio|0)))},exports.doMenu=function(elem){}');
bundler.module('scripts/combigame.js','var $=require("zquery"),_=require("underscore"),webutil=require("webutil"),fullbrows=require("fullbrows"),Modernizr=require("modernizr"),hidden={opacity:0,width:0,height:0},transitionStyle={transition:"opacity 1s","-moz-transition":"opacity 1s","-webkit-transition":"opacity 1s","-o-transition":"opacity 1s"},visibleStyle,selectedStyle,unselectedStyle,pos,size,difficulty,doLayout=function(){var $content=$("#content");$content.css("background","white");var w=$content.width(),h=$content.height(),topPad,leftPad,landscape=!0,i,x,y;pos=[],size=Math.min(Math.max(w,h)/4,Math.min(w,h)/3)*.85,w>h?(topPad=h-size*3>>1,leftPad=w-size*4>>1):(topPad=h-size*4>>1,leftPad=w-size*3>>1,landscape=!1),topPad+=.05*size,leftPad+=.05*size;for(i=0;i<12;++i)landscape?pos.push({top:topPad+(.5+i%3)*size,left:leftPad+(.5+(i/3|0))*size}):pos.push({left:leftPad+(.5+i%3)*size,top:topPad+(.5+(i/3|0))*size});visibleStyle={opacity:1,"margin-top":-size/2,"margin-left":-size/2,width:.9*size,height:.9*size},selectedStyle={"border-style":"solid","border-width":1,"margin-top":-size/2-1,"margin-left":-size/2-1,"border-radius":size/16,"border-color":"gray"},unselectedStyle={"margin-top":-size/2,"margin-left":-size/2,border:"none"},$(".card").css({top:h,left:w});for(i=0;i<12;++i)anim(i,$("#card"+cards[i]))();$(".menuIcon").css({width:size/1.5,height:size/1.5}),$(".menuEast").css({left:w-size/1.5}),$(".menuSouth").css({top:h-size/1.5}),$(".difficultyStatus").css({position:"absolute",width:size/2.3,top:size/1.5,"text-align":"right",height:size*2,left:w-size/2}),webutil.scaleText($(".difficultyStatus")),$(".difficultyStatus").css("overflow","visible")},lastClickTime=0,prevCard="";function click(card){if(card===prevCard&&Date.now()-lastClickTime<100)return;prevCard=card,lastClickTime=Date.now(),exports.selected[card]?($("#card"+card).css(unselectedStyle),delete exports.selected[card]):($("#card"+card).css(selectedStyle),exports.selected[card]=!0),testSelected()}function shuffle(fn){var score,bestscore=-1e4,saved,i;for(i=0;i<10;++i){do fn(),score=okDeck();while(!score);if(difficulty==="normal"){saved=cards;break}difficulty==="hard"&&(score=-score),score>bestscore&&(saved=cards.slice(0),bestscore=score)}cards=saved}var logData,curDate;function log(obj){setTimeout(function(){var objDate=obj.now/24/60/60/1e3|0;objDate!==curDate&&(curDate=objDate,logData=JSON.parse(localStorage.getItem("combigamelog"+curDate)||"[]")),logData=logData||[],logData.push(obj),localStorage.setItem("combigamelog"+curDate,JSON.stringify(logData))},0)}function partialScore($t,title,log){log.length>0&&(title&&$t.append($("<div><b>"+title+"</b></div>")),$t.append($("<div>Best time: "+(log[0].time/10|0)/100+"s")),$t.append($("<div>Median time: "+(log[log.length>>1].time/10|0)/100+"s")))}function showScore(){fullbrows.start({update:function(){var $t=$("<div>"),log=_(logData).filter(function(elem){return!elem.hint&&elem.difficulty===difficulty}).sort(function(a,b){return a.time-b.time});$t.append($("<h3>Timings\xa0"+difficulty+"</h3>")),log.length===0&&$t.append("<p>No score available for this difficulty, please play the game before looking at the timings.</p>"),partialScore($t,"Today",log),partialScore($t,"Last five minutes",log.filter(function(elem){return Date.now()-elem.now<3e5})),partialScore($t,"Last minute",log.filter(function(elem){return Date.now()-elem.now<6e4})),$t.append("<p>Click to close.</p>"),$("#content").html("").append($t),$t.css({width:"80%",height:"90%"}),webutil.scaleText($t),$t.css({margin:"3% 10% 7% 10%",overflow:"visible"}),$t.bind("mousedown touchstart",fullbrows.startFn(exports.app))}})}function testSelected(){var list=Object.keys(exports.selected);if(list.length>=3){if(okSet(list[0],list[1],list[2])){var now=Date.now();log({time:now-prevtime,hint:giveup,cards:cards.slice(0),choosen:list,now:now,difficulty:difficulty}),giveup=!1,prevtime=now,setTimeout(function(){list.forEach(function(id){$("#card"+id).css("opacity",0)})},0),setTimeout(function(){$(".card").css(unselectedStyle);var ids=[_(cards).indexOf(list[0]),_(cards).indexOf(list[1]),_(cards).indexOf(list[2])];shuffle(function(){for(var i=0;i<3;++i)cards[ids[i]]=randomCard()}),doLayout()},1e3)}else $(".card").css(unselectedStyle);exports.selected={}}}exports.selected={};function anim(i,$card){return function(){$card.css(pos[i]),setTimeout(function(){$card.css(transitionStyle).css(visibleStyle)},0)}}function rnd3(){return Math.random()*3|0}function randomCard(){return""+rnd3()+rnd3()+rnd3()+rnd3()}var cards=[];function okSet(a,b,c){for(var i=0;i<4;++i){var ok=a[i]===b[i]&&b[i]===c[i]||a[i]!==b[i]&&b[i]!==c[i]&&c[i]!==a[i];if(!ok)return!1}return!0}function hint(){exports.selected={},$(".card").css(unselectedStyle);var a,b,c;for(a=0;a<10;++a)for(b=a+1;b<11;++b)for(c=b+1;c<12;++c)if(okSet(cards[a],cards[b],cards[c])){$("#card"+cards[a]).css(selectedStyle).css({opacity:.6,border:"1px solid #ddd"}),$("#card"+cards[b]).css(selectedStyle).css({opacity:.6,border:"1px solid #ddd"}),$("#card"+cards[c]).css(selectedStyle).css({opacity:.6,border:"1px solid #ddd"}),giveup=!0;return}}function okDeck(){var cardHash={},a,b,c,i,ok=0;for(i=0;i<12;++i){if(cardHash[cards[i]])return!1;cardHash[cards[i]]=!0}for(a=0;a<10;++a)for(b=a+1;b<11;++b)for(c=b+1;c<12;++c)okSet(cards[a],cards[b],cards[c])&&++ok;return ok}var prevtime,giveup;function startGame(){giveup=!1;var $content=$("#content");$content.html(""),prevtime=Date.now();var i,j,k,l;for(i=0;i<3;++i)for(j=0;j<3;++j)for(k=0;k<3;++k)for(l=0;l<3;++l)$content.append($(\x27<img class="card" src="dist/combigame\x27+i+j+k+l+\x27.png" id="card\x27+i+j+k+l+\x27">\x27));$(".card").css({position:"absolute",opacity:"0"}),$(".card").bind("touchstart mousedown",function(e){return click(e.target.id.slice(4)),e.preventDefault(),!0}),$content.append($(\x27<img class="menuIcon" src="img/help.png" alt="How to play">\x27).css("position","absolute").bind("click",function(){fullbrows.start({update:function(){var html=require("jsxml").toXml(["div",["h2","CombiGame"],"Game objectives: click on combinations of three figures where color, count, shape, and fill, are either the same or all different.",["br"],["p",["img",{style:"height: 1.5em; width: 1.5em;",src:"img/give-up.png"}]," shows a valid combination among the figures.",["br"],["img",{style:"height: 1.5em; width: 1.5em;",src:"img/difficulty.png"}]," sets difficulty \u2022\xa0easy    : ca. 6 valid combinations \u2022\xa0hard: 1 valid combination \u2022\xa0normal: random number of valid combinations.",["br"],["img",{style:"height: 1.5em; width: 1.5em;",src:"img/score.png"}]," shows latest timing for the current difficulty.",["br"]],"Click to close."]),$t=$("<div>");$t.html(html),$content.html("").append($t),$t.css({width:"90%",height:"90%"}),webutil.scaleText($t),$t.css({margin:"2% 5% 8% 5%",overflow:"visible"}),$t.bind("mousedown touchstart",fullbrows.startFn(exports.app))}})})).append($(\x27<img class="menuIcon menuEast" src="img/difficulty.png" alt="Difficulty">\x27).css("position","absolute").bind("click",function(){menu({easy:function(){difficulty="easy",fullbrows.start(exports.app)},normal:function(){difficulty="normal",fullbrows.start(exports.app)},hard:function(){difficulty="hard",fullbrows.start(exports.app)}})})).append($(\x27<img class="menuIcon menuSouth" src="img/give-up.png" alt="Give up">\x27).css("position","absolute").bind("click",hint)).append($(\x27<img class="menuIcon menuEast menuSouth" src="img/score.png" alt="Score">\x27).css("position","absolute").bind("click",showScore)),difficulty=difficulty||localStorage.getItem("combigameDifficulty")||"normal",localStorage.setItem("combigameDifficulty",difficulty),$("#content").append($(\x27<div class="difficultyStatus">\x27).text(difficulty)),shuffle(function(){cards=[];for(var i=0;i<12;++i)cards.push(randomCard())}),doLayout()}function menu(items){fullbrows.start({update:function(){var item,s=Math.min($("#content").height()+$("#content").width()),$menu=$("<div>"),$content=$("#content");$content.html("").append($menu);for(item in items)$menu.append($("<div>").text(item).css({border:"1px solid black","border-radius":s*.02,"text-align":"center",margin:s*.01,padding:s*.01}).bind("click",items[item]));webutil.scaleText($content),$content.css("font-size",parseInt($content.css("font-size"),10)*.8),$menu.css("top",($content.height()-$menu.height())/2),$menu.css("position","absolute"),$menu.css("width","100%")}})}exports.app={start:startGame,update:doLayout}');
...
bundler.module('scripts/fullbrows.js','var util=require("util"),webutil=require("webutil"),$=require("zquery"),window=require("window"),Modernizr=require("modernizr"),app={};function relayoutStyle(){$("#content").css("position","absolute").css("left",0).css("top",1).css("overflow","hidden").css("width","100%").css("height",app.type==="scrollable"?"auto":webutil.windowHeight()),window.scrollTo(0,1)}function relayout(){relayoutStyle(),typeof app.update=="function"&&app.update()}var relayoutDelayed=util.niceSingle(relayout),isMobile=window.isMobile||!!window.navigator.userAgent.match(/mobile/i);exports.start=function(opt){typeof app.stop=="function"&&app.stop(),app=opt||{},app.type=app.type||"fullscreen",app.mobile===undefined&&(app.mobile=isMobile),$("#content").remove(),app.type==="canvas"?$("body").append(\x27<canvas id="content">Error: canvas not supported, please update to a modern browser.</canvas>\x27):$("body").append(\x27<div id="content"></div>\x27),app.$=$("#content"),app.elem=app.$[0],Modernizr.touch?($("#addressbarfillerdisable").remove(),$("body").append(\x27<div id="addressbarfillerdisable" style="height:\x27+(Math.max(webutil.windowHeight(),$(window).width())+62)+\x27px;"></div>\x27)):app.type==="scrollable"?$("body").css("overflow","auto"):$("body").css("overflow","hidden"),$(window).bind("resize",relayoutDelayed),$(window).bind("orientationchange",relayoutDelayed),app.start=app.start||app.update,relayoutStyle(),app.start()},exports.startFn=function(app){return console.log(1),function(){console.log(2),exports.start(app)}}');
bundler.require("main").main()
...
bundler.module('scripts/combigame.js','var cardSprites,$=require("zquery"),_=require("underscore"),Modernizr=require("modernizr"),hidden={opacity:0,width:0,height:0},transitionStyle={transition:"opacity 2s","-moz-transition":"opacity 2s","-webkit-transition":"opacity 2s","-o-transition":"opacity 2s"},visibleStyle,selectedStyle,unselectedStyle,pos,size,doLayout=function(){var $content=$("#content");$content.css("background","white");var w=$content.width(),h=$content.height(),topPad,leftPad,landscape=!0,i,x,y;pos=[],size=Math.min(Math.max(w,h)/4,Math.min(w,h)/3),w>h?(topPad=h-size*3>>1,leftPad=w-size*4>>1):(topPad=h-size*4>>1,leftPad=w-size*3>>1,landscape=!1),topPad+=.05*size,leftPad+=.05*size;for(x=0;x<3;++x)for(y=0;y<3;++y)pos.push({top:topPad+(.5+y)*size,left:leftPad+(.5+x)*size});for(i=0;i<3;++i)landscape?pos.push({top:topPad+(.5+i)*size,left:leftPad+3.5*size}):pos.push({top:topPad+3.5*size,left:leftPad+(.5+i)*size});visibleStyle={opacity:1,"margin-top":-size/2,"margin-left":-size/2,width:.9*size,height:.9*size},selectedStyle={"border-style":"solid","border-width":1,"margin-top":-size/2-1,"margin-left":-size/2-1,"border-radius":size/16,"border-color":"gray"},unselectedStyle={"margin-top":-size/2,"margin-left":-size/2,border:"none"},!!cardSprites,$("#timer").css({"font-size":w/3,color:"#eeeeee",position:"absolute","text-align":"center",width:"100%"}).css("top",(h-$("#timer").height())/2),$(".card").css({top:-size,left:-size});for(i=0;i<12;++i)anim(i,$("#card"+cards[i]))()},lastClickTime=0,prevCard="";function click(card){if(card===prevCard&&Date.now()-lastClickTime<1e3)return;prevCard=card,lastClickTime=Date.now(),console.log(card),exports.selected[card]?($("#card"+card).css(unselectedStyle),delete exports.selected[card]):($("#card"+card).css(selectedStyle),exports.selected[card]=!0),testSelected()}var score=0;function testSelected(){var list=Object.keys(exports.selected);list.length>=3&&(okSet(list[0],list[1],list[2])&&(++score,setTimeout(function(){list.forEach(function(id){$("#card"+id).css("opacity",0)})},0),setTimeout(function(){var ids=[_(cards).indexOf(list[0]),_(cards).indexOf(list[1]),_(cards).indexOf(list[2])];do for(var i=0;i<3;++i)cards[ids[i]]=randomCard();while(!okDeck());doLayout()},2e3)),$(".card").css(unselectedStyle),exports.selected={})}exports.selected={};function anim(i,$card){return function(){$card.css(pos[i]),setTimeout(function(){$card.css(transitionStyle).css(visibleStyle)},0)}}function rnd3(){return Math.random()*3|0}function randomCard(){return""+rnd3()+rnd3()+rnd3()+rnd3()}var cards=[];function okSet(a,b,c){for(var i=0;i<4;++i){var ok=a[i]===b[i]&&b[i]===c[i]||a[i]!==b[i]&&b[i]!==c[i]&&c[i]!==a[i];if(!ok)return!1}return!0}function okDeck(){var cardHash={},a,b,c,i;for(i=0;i<12;++i){if(cardHash[cards[i]])return!1;cardHash[cards[i]]=!0}for(a=0;a<10;++a)for(b=a+1;b<11;++b)for(c=b+1;c<12;++c)if(okSet(cards[a],cards[b],cards[c]))return!0;return!1}var finalTime;function timer(){var now=Date.now();if(now>finalTime)alert("Score: "+score),menu();else{var sec=(finalTime-now)/1e3;$("#timer").text((sec/60|0)+":"+(""+(100+sec%60)).slice(1,3)),setTimeout(timer,100)}}function startGame(){$("#content").html(\x27<div id="timer"></div>\x27),finalTime=Date.now()+18e4,timer(),require("combigameCards").createCards($("#content").width()/3|0),$(".card").bind("touchstart mousedown",function(e){return click(e.target.id.slice(4)),!0}),score=0;do{cards=[];for(var i=0;i<12;++i)cards.push(randomCard())}while(!okDeck());require("fullbrows").init({update:doLayout})}function menu(){$("#content").html("").append($("<div>").text("Play").bind("click",startGame)).append($("<div>").text("Settings").bind("click",settings)).append($("<div>").text("Score").bind("click",showScore)).append($("<div>").text("Help").bind("click",help)).css({"font-family":"sans-serif","text-align":"center",width:"80%",height:"80%",left:"10%",top:"10%"}),$("#content>div").css({}),require("webutil").scaleText($("#content")),$("#content").css("overflow","visible")}function help(){}function settings(){}function showScore(){}exports.run=function(){require("fullbrows").init({update:menu})}');
